...
bool TextClass::SetMousePosition(int mouseX, int mouseY, ID3D11DeviceContext* deviceContext)
{
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence4, mouseString, 120, 60, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetScreenSize(int screenWidth, int screenHeight, ID3D11DeviceContext* deviceContext)
{
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence5, ScreenString, 20, 80, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence6, ScreenString, 140, 80, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetNumPolygon(int numP, ID3D11DeviceContext* deviceContext)
{
	char tempString[16];
	char PolyString[32];
	bool result;
	...
	// Convert the mouseX integer to string format.
	_itoa_s(numP, tempString, 10);

	// Setup the mouseX string.
	strcpy_s(PolyString, "Num of Polygon = ");
	strcat_s(PolyString, tempString);

	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence7, PolyString, 20, 100, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}

	_itoa_s(216, tempString, 10);

	// Setup the mouseX string.
	strcpy_s(PolyString, "Num of Object = ");
	strcat_s(PolyString, tempString);

	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence8, PolyString, 20, 120, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
